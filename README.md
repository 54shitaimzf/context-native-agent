...
一篇约 2.4 万字的中文长文（摘要 + 序言 + 正文 + 三个附录），讨论 LLM Agent 的上下文成本，以及围绕这笔成本该怎么设计 Agent 架构。
...
v1.5, 2026-09-16. https://github.com/54shitaimzf/context-native-agent
```
...
最后一次交付的校验输出：Markdown 与 HTML 逐行比对 494 行、未命中 0；引用 124 处、52 条参考文献双向可达；25 张表列数一致；134 个内部链接无悬空；3 张图（含一张手绘的架构拓扑图）；成品 A4 38 页、209 个链接，文字落在版心内、图内文字越界 0 处、表格横向溢出 0 张，全篇底部留白合计 421mm（均 11.1mm/页，只有目录页超过 30mm）。
