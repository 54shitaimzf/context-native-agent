...
一篇约 2.4 万字的中文长文（序言 + 正文 + 三个附录），讨论 LLM Agent 的上下文成本，以及围绕这笔成本该怎么设计 Agent 架构。
...
换掉的不只是措辞，而是**问题的解空间**：上下文从"必须被管理的缓冲区"变成"可以从环境重建的投影"，压缩于是不再是唯一的出路。由此推出的两条，是全文的骨架：
...
这笔账刻意往对自己不利的方向取——被摘要丢掉、后来又得重新推一遍的内容，多出来的协调步骤，质量上的好处，全都不算钱；不利的量一律往大取。只有**四条例外**（两条能力假设、一条越过了真实机制的取法、一条量级 0.6 个百分点的取舍），单独列在附录2 的 A2.7 里，各自交代把结论推到哪一步。所以它不是"刚好划算"，而是在压着自己的条件下仍然划算。

要让这笔账成立，设计上得守住三件事：每轮常驻的上下文压得住，每轮为更新它而真正写出去的量足够小，内容没变的那部分能一直复用。正文给了这三个量的算法，全部参数与取法列在附录2。**这笔账量的是文中那处具体实现（Git 树）；换一处实现、换一组参数，数字会变，而四个台阶、三个判据、有效残留这几样东西的形状不变。**
...
附录3 里的【原文】引用，文字权利仍属原作者；本仓库的许可只覆盖作者自己的表达。正文在 v1.5.2 做过四处经作者批准的改动（删掉整节摘要、解空间那一节前移、两处措辞换字），其余改动只有标点规范化，逐条见 `REVISION.md` 第一节。
...
最后一次交付的校验输出：Markdown 与 HTML 逐行比对 493 行、未命中 0；引用 124 处、52 条参考文献双向可达；25 张表列数一致；134 个内部链接无悬空；3 张图（含一张手绘的架构拓扑图）；成品 A4 38 页、209 个链接，文字落在版心内、图内文字越界 0 处、表格横向溢出 0 张，全篇底部留白合计 471mm（均 12.4mm/页；超过 30mm 的 4 页是目录页与三张图各自的前一页）。
