...
- **页边注的两次转折**：屏幕上是正文右侧留白里的真页边注（与所注段落首行齐平）；但 **Chrome 导出 PDF 时会丢弃落在版心外的绝对定位元素**（实测三种 absolute/float 写法全部丢失），所以打印时改为流内的一条右对齐灰色小注，**并强制插在区块末尾**——插在开头会把列表项拆成"孤立项目符号 + 换行正文"。
- **页码**：目录页码用 `pdftotext` 反查成品 PDF 得到（先导出→反查→重建 HTML→再导出，迭代到两轮一致，已收敛）；页脚由导出脚本注入「当前页 / 总页数」。页脚框是整幅纸宽、不跟 `@page` 边距走，需自己补内边距，否则页码会贴到纸边。
- **末页**：colophon 前加「版式说明」小标，字号降到 11px、灰色，避免被当成正文。
...
node normalize-punctuation.mjs   # 标点规范化（干跑；加 --write 才写盘）
```
...
截至目前，**正文只做过作者点头的三处标点修正**（`19b2a93` 之后）：
...
**还有一类未决**：源文件里另有 **372 个 ASCII 直引号**（正文 4 行 10 个；附录2 71 行 256 个；附录3 33 行 106 个），中文排版应换成 `“ ”`。已经写好工具并干跑验证：

```bash
node normalize-punctuation.mjs            # 干跑，逐行打印将要做出的替换
node normalize-punctuation.mjs --write    # 写入
```

干跑结果：**108 行 / 372 个引号，0 行因"引号个数为奇数"被跳过**（即全部成对，可机械替换，不会把开引号写成闭引号）；没有引号落在行内代码或数学公式里。是否替换等作者定夺——附录部分本可自由改，但只改附录会让同一篇文档里出现两种引号字形，所以要么都改、要么都不改。
