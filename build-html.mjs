...
for (const b of blocks) {
  ...
  if (b.t === 'p') {
    ...
    if (curSection.startsWith('正文') && plain.startsWith('以这一理念出发')) {
      out.push(figure(chartArch(), '图 2　一轮的动作。四段前缀按此顺序固定：前两段整轮逐字节不变，后两段（意图映射、系统状态）每版更新，所以未命中只落在后两段上。环境侧底层内容只有一份，每个分支一份虚拟工作区、写时才占空间。合并由主 Agent 以单写者执行，依据是它制定契约时的那份上下文；分支的上下文随分支消失，原文留在 git 里；而“共享同一版字节”以各分支同模型为前提（前缀单元按模型分家，见 A3.6）。数据源：正文“具体实现”一节、附录2 A2.4。'));
    }
  ...
  if (b.t === 'table') {
    ...
    if (isCostTable) out.push(figure(chartAmortize(), '图 4　共享的摊薄形状。一版前缀在同一轮内被所有分支逐字节复用，未命中只发生在换代那一次、且只落在变动的那一段上——所以它按前缀版本数计，不按分支数计；左侧那行括注写的就是“若按分支计”会差成什么样。右侧只画“重读次数”的形状，不含新数字；这一档以各分支同模型为前提——前缀单元按模型分家，跨模型时吃不到彼此的缓存（见 A3.6）。数据源：附录2 A2.4 的重置项。'));
